...
public enum VehicleHud {
    ...
    private static void renderSpeedometer(DrawContext context, VehicleEntity vehicle) {
        ...
        context.drawTextWithShadow(MinecraftClient.getInstance().textRenderer, Text.literal(AUtils.DEC_TWO_PLACES.format(convert(speed)) + " km/h"), 20, 20, color);
    }
    ...
    private static void renderControlHints(DrawContext context, float alpha) {
        ...
        for (var control : CONTROL_HINTS) {
            ...
            context.drawTextWithShadow(font, keyTxt, x + 3, y + 3, textColor);
            context.drawTextWithShadow(font, control.getText(), x + keyTxtWid + 9, y + 3, textColor);
